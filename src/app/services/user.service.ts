...
export class UserService {
  ...
  setUserData() {
    ...
    this.user$.subscribe((auth: Auth0User) => {
      ...
      this.http
        .post("/api/users/userinfo", authuser)
          ...
          if (response.success) {
            ...
            if (data.first_name != authfirst || data.last_name != authlast) {
              ...
              this.http.put("/api/users/edit", fixed).subscribe((res) => {
                this.snackbar.open(
                  "Profile updated to match your Google profile.",
                  "OK"
                );
              });
            }
            ...
            this.snackbar.open(
              "Your profile could not be retrieved from the database.",
              "Close",
              { duration: 3000 }
            );
        });
  ...
  getInitialData(roleid: string, id: string) {
    ...
    if (roleid === "1") {
      ...
      this.cohorts.getCohortEnrollment(), this.lessons.getAllLessons();
